...
export function EventContextProvider({ children }) {
    const [EventState , dispatchEvent] = useReducer(EventReducerFunc , InitialstateEvent)
    ...
    function stateClosing() {
        const stateClosing = {
            type: 'dropdown',
            data: {
                searchPreset: `${EventState.searchPreset == 'open' ? 'closing' : 'close'}`, 
                dropdown: `${EventState.dropdown == 'open' ? 'closing' : 'close'}`, 
                backdrop: `${EventState.backdrop == 'open' ? 'closing' : 'close'}`, 
                searchFocus: ''
            }
        }
    ...
    function stateClose() {
        const stateClose = {
            type: 'dropdown',
            data: {
                searchPreset: `${EventState.searchPreset == 'open' ? 'close' : 'close'}`, 
                dropdown: `${EventState.dropdown == 'open' ? 'close' : 'close'}`, 
                backdrop: `${EventState.dropdown == 'open' ? 'close' : 'close'}`, 
                searchFocus: ''
            }
        }
