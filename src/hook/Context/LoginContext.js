...
export function LoginContextProvider({ children }) {
    const [LoginState , dispatchLogin] = useReducer(LoginReducerFunc , InitialstateLogin)
    
    function handleInputChange(event) {
        return {type: event.target.name , data: {name: event.target.name , value: event.target.value}}
    }
    ...
    function FormErrorSubmit(LoginState , errorObj) {
        if (errorObj.emailError == '' && errorObj.passwordError == '' && errorObj.usernameError == '') {
            return {
                type: 'reset', 
                reset: {
                    email: '',
    ...
    function handleShowPassword(LoginState) {
        ...
        return {
            type: 'showPassword', 
            showObj: {showPassword: LoginState.showPassword.id === 'show' ? hidden : show}
        }
    }
